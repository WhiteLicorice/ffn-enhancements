    ...
    submitReplaceForm: async function (actionUrl: string, storyTextId: string, docId: string): Promise<StoryReplaceResult> {
        ...
        return new Promise<StoryReplaceResult>((resolve) => {
            ...
            try {
                ...
                // GOTCHA: FFN's replace form can carry per-page hidden auth/CSRF
                // fields. Building a bare form without them triggers 403 from
                // Cloudflare. Copy all hidden inputs except the ones we
