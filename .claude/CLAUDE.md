...
- Vite bundles everything (including `jszip`, `file-saver`, `turndown`, and
  `marked`) into one self-contained userscript file. There are no lazy chunks
  and no `externalGlobals` mapping.
- `vite-plugin-monkey` defaults `build.minify` to `false`, which is a footgun
  for this project. Keep `build.minify: 'esbuild'` in `vite.config.ts` for
  production builds so Tampermonkey does not have to parse a much larger bundle
  before the paint-gate prelude can run.
- The userscript has a single `@require` entry: a `data:` URL paint-gate
  prelude generated by `util.fn2dataUrl(...)` in `vite.config.ts`. It must stay
  the only `@require`. Do not add CDN `@require` entries - Tampermonkey blocks
  userscript injection until every `@require` resolves, so extra `@require`
  entries delay the prelude, worsen the FOUC race, and reintroduce a visible
  unstyled flash. Lessons learned: see plan `unified-sauteeing-trinket.md`.
- `vite.config.ts` also sets `userscript.noframes = true`. Do not depend on the
  userscript running inside iframes; TinyMCE iframe theming is driven from the
  parent frame via `ThemeManager`.
- Chrome + Tampermonkey users who care about first-paint theming should enable
  Dashboard -> Settings -> Experimental -> Inject Mode = `Instant`. MV3 injects
  content scripts with a residual delay that the script cannot fully remove on
  its own.
...
15. **GOTCHA: Minify and `@noframes` are load-path requirements.** The production
    userscript depends on `build.minify: 'esbuild'` to keep parse time low enough
    for the paint gate to win the first-paint race, and on `userscript.noframes`
    to avoid re-parsing the whole bundle inside TinyMCE iframes. Treat both as
    functional requirements, not optional cleanup.
